...
def parse_test_response(content: str, expected_test_file: str) -> dict:
    ...
    if missing:
        raise ValueError(f"generated test result is missing required keys: {', '.join(missing)}")
    if str(parsed['test_file']).strip()!=expected_test_file:
        raise ValueError(f"generated test result field 'test_file' must be exactly '{expected_test_file}'")
    return {'test_file': expected_test_file, 'code': parsed['code']}
