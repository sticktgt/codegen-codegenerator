...
`codegenerator` — внешний генератор для `codecollector`, который принимает структурированный request, собирает prompt, вызывает модель через Ollama-compatible endpoint и возвращает нормализованный результат генерации кода, теста или repair.
...
### 4. Вызов модели
Модель вызывается через Ollama-compatible endpoint с параметрами из `config.yaml`. Текущая демонстрационная конфигурация ориентирована на Qwen-модели через удаленный endpoint; локальная Ollama остается допустимым fallback-сценарием.
...
5. reference artifacts как дополнительный источник стиля и проектных паттернов;
6. `example_test_source` только как fallback-источник.
...
- reference artifacts могут входить в `generate-test` как дополнительный справочный контекст, но не должны подменять `generated_code_artifact`, `related_tests` и `full_file_source`.
...
- reference artifacts, если они переданы и помещаются в budget;
...
- модели вызываются через Ollama-compatible endpoint; текущая демонстрационная конфигурация использует Qwen-модели через удаленный endpoint, локальная Ollama остается fallback-сценарием;
...
`codegenerator` в текущем состоянии — это внешний генератор кода, тестов и repair-артефактов, который получает уже подготовленный структурированный request, управляет runtime budget, вызывает модель через Ollama-compatible endpoint, нормализует результат и возвращает его в стабильном машиночитаемом формате.
